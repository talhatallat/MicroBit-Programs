...
int lsm303_ll_writeRegisterMag(uint8_t RegNum, uint8_t Value);
...
int lsm303_ll_begin()
{
	...
	lsm303_ll_writeRegisterMag(0x60,0x80);
	lsm303_ll_writeRegisterMag(0x10,0x62);
...
int lsm303_ll_readMagX() // returns Temperature * 100
{
	...
	uint8_t buf[2];
	buf[0] = 0x80+0x2a;	
	i2c_burst_read(i2c,LSM303_MAG_ADDRESS,0xe6, buf,2);
	accel = buf[1];
	accel = accel << 8;
	accel = accel + buf[0];
...
int lsm303_ll_readRegisterMag(uint8_t RegNum, uint8_t *Value)
{
	    //reads a byte from a specific register
    int nack;   
	nack=i2c_reg_read_byte(i2c,LSM303_MAG_ADDRESS,RegNum,Value);
	return nack;
}
